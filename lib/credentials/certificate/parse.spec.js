...
describe('parseCredentials', () => {
	...
	describe('with PFX file', () => {
		...
		describe('having passphrase', () => {
			beforeEach(() => {
				fakes.parsePkcs12.mockReturnValue({ key: pfxKey, certificates: [pfxCert] })
				fakes.parsePkcs12.mockImplementation(() => { throw new Error('unable to read credentials, incorrect passphrase') })
			})
	...
	describe('with PEM key', () => {
		...
		describe('having passphrase', () => {
			beforeEach(() => {
				fakes.parsePemKey.mockReturnValue(pemKey)
				fakes.parsePemKey.mockImplementation(() => { throw new Error('unable to load key, incorrect passphrase') })
			})
