...
describe('prepareToken', () => {
  ...
  describe('with bad `key` parameter', () => {
    ...
    test('attaches the original resolve error as `cause`', () => {
      expect.assertions(2)
      try {
        prepareToken(testOptions)
      } catch (e) {
        expect(e.cause).toBeInstanceOf(Error)
        expect(e.cause.message).toMatch(/ENOENT/)
      }
    })
  })

  describe('when token signing fails', () => {
    beforeEach(() => {
      mockDependencies.sign.mockImplementation(() => {
        throw new Error('signing exploded')
      })
    })

    test('throws a wrapped "Failed to generate token" error', () => {
      expect(() => prepareToken(testOptions)).toThrow(/Failed to generate token/)
    })

    test('attaches the original sign error as `cause`', () => {
      expect.assertions(2)
      try {
        prepareToken(testOptions)
      } catch (e) {
        expect(e.cause).toBeInstanceOf(Error)
        expect(e.cause.message).toBe('signing exploded')
      }
    })
  })
})
