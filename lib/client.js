...
module.exports = function (dependencies) {
  ...
  Client.prototype.write = function write(notification, device, count) {
    if (this.isDestroyed) {
      return Promise.resolve({ device, error: new Error('client is destroyed') })
    }
  ...
  Client.prototype.request = function request(notification, device, count) {
    ...
    return new Promise((resolve) => {
        ...
        try {
          ...
          if (status === 200) {
            ...
            if (status === 403 && response.reason === 'ExpiredProviderToken' && retryCount < 2) {
              ...
              const error = new Error('Error 500, stream ended unexpectedly')
            ...
            this.closeAndDestroySession()
            const error = new Error(`stream ended unexpectedly with status ${status} and empty body`)
            resolve({ device, error })
          }
        } catch (e) {
          const error = new Error('Unexpected error processing APNs response')
          if (this.errorLogger.enabled) {
      ...
      request.setTimeout(this.config.requestTimeout, () => {
        ...
        resolve({ device, error: new Error('apn write timeout') })
      })
      ...
      request.on('aborted', () => {
        ...
        resolve({ device, error: new Error('apn write aborted') })
      })
      ...
      request.on('error', (error) => {
        ...
        if (typeof error === 'string') {
          error = `apn write failed: ${error.message ?? error.code ?? ''}`
        } else {
          error = `${error.message ?? error.code ?? ''} apn write failed`
        }
