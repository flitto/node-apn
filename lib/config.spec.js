const { describe, expect, test, beforeEach, afterEach, jest: j } = require('@jest/globals')
  ...
  describe('credentials', () => {
    describe('`tokenSpec` not supplied, use certificate', () => {
      ...
      describe('pfxData value is supplied along with a cert and key', () => {
        test('includes the value of `pfxData`', () => {
          expect(config({ pfxData: 'apnData', cert: 'cert.pem', key: 'key.pem' })).toHaveProperty('pfxData', 'apnData')
        })
      ...
      test('loads and validates the TLS credentials', () => {
        fakes.prepareCertificate.mockReturnValue({ cert: 'certData', key: 'keyData', pfx: 'pfxData' })
      ...
      test('prepares the CA certificates', () => {
        fakes.prepareCA.mockReturnValue({ ca: 'certificate1' })
    ...
    describe('`token` supplied', () => {
      ...
      test('clears the `pfx` property', () => {
        expect(config({ token: { key, keyId, teamId } })).not.toHaveProperty('pfx')
      })

      test('clears the `key` property', () => {
        expect(config({ token: { key, keyId, teamId } })).not.toHaveProperty('key')
      })

      test('clears the `cert` property', () => {
        expect(config({ token: { key, keyId, teamId } })).not.toHaveProperty('cert')
      })

      describe('token', () => {
        test('throws an error if keyId is missing', () => {
          expect(() => config({ token: { key, teamId } })).toThrow(/token\.keyId is missing/)
        })

        test('throws an error if keyId is not a string', () => {
          expect(() => config({ token: { key, teamId, keyId: 123 } })).toThrow(/token\.keyId must be a string/)
        })

        test('throws an error if teamId is missing', () => {
          expect(() => config({ token: { key, keyId } })).toThrow(/token\.teamId is missing/)
        })

        test('throws an error if teamId is not a string', () => {
          expect(() => config({ token: { key, keyId, teamId: 123 } })).toThrow(/token\.teamId must be a string/)
        })
      })
