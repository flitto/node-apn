...
describe('Client', () => {
  ...
  test('Treats HTTP 200 responses as successful', async () => {
    ...
    expect(establishedConnections).toEqual(0)
    ...
    expect(establishedConnections).toEqual(1)
  ...
  test('Closes connections when HTTP 500 responses are received', async () => {
    ...
    const runRequestWithInternalServerError = async () => {
      ...
      expect(result.error.message).toEqual('Error 500, stream ended unexpectedly')
    }
  ...
  test('Handles APNs timeouts', async () => {
    ...
    clientServer = createAndStartMockServer(TEST_PORT, (req, res) => {
      didGetRequest = true
      setTimeout(() => {
        res.writeHead(200)
        res.end('')
